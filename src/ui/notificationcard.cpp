...
NotificationCard::NotificationCard(const Notification& n, const Config& cfg, QWidget* parent)
    : QWidget(parent),
      m_id(n.id),
      m_cfg(cfg) {
    ...
    if (m_timed) {
        ...
        connect(m_lifeTimer, &QTimer::timeout, this, &NotificationCard::onTimeoutFinished);
        m_lifeTimer->start();
