...
# Apply a preset by simply copying it into the theme layer. The whole preset
# file becomes symm.theme.ini; the user layer (symm.user.ini) still overrides
# on top. No section-merging.
merge_into_theme() {
  local src="$1"
  cp -f "$src" "$THEME_OUT"
}
